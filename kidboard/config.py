...
# repeat presses of the same key within this many milliseconds.
#
# 100 ms, set from watching the real board rather than a bench sample - the
# fault is intermittent and wanders between keys, so short measurements
# understate it. The cost of erring high is invisible here: nobody types on this
# thing, and losing a genuine same-key repeat inside 100 ms would not be
# noticeable in a light toy. Set 0 to disable and see the raw stream.
DEBOUNCE_MS = _int("KIDBOARD_DEBOUNCE_MS", 100)
